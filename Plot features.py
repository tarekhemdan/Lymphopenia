...
for col in adni_df.columns[:-1]: # Exclude the Status variable
    fig, ax = plt.subplots()
    sns.histplot(data=adni_df, x=col, hue='albumin 2', kde=True, ax=ax)
...
for col in adni_df.columns[:-1]: # Exclude the Status variable
    fig, ax = plt.subplots()
    sns.boxplot(data=adni_df, x='albumin 2', y=col, ax=ax)
...
sns.pairplot(adni_df, hue='albumin 2')
...
for col in adni_df.columns[:-1]: # Exclude the Status variable
    fig, ax = plt.subplots()
    sns.violinplot(data=adni_df, x='albumin 2', y=col, ax=ax)
...
for col in adni_df.columns[:-1]: # Exclude the Status variable
    fig, ax = plt.subplots()
    sns.scatterplot(data=adni_df, x=col, y='albumin 2', ax=ax)
    ax.set_title(f"Scatter plot of {col} vs. albumin 2")
